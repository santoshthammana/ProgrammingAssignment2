# Matrix inversion can be time intensive for a large matrix.
# The functions below allow the results of matrix inversion to be cached for quick retrieval.

## makeCacheMatrix takes in originalMatrix as input and returns a "cache matrix",
## which is a list of functions allowing for a matrix inverse to be cached.
...
## cacheSolve returns the inverse of the matrix stored in cacheMatrix.
## If cached, the inverse is simply retrieved.  Otherwise, the inverse is calculated and cached.
...
cacheSolve <- function(cacheMatrix, ...) {
  ...
  ## Cache the matrix inverse in cacheMatrix.
  ## If the original matrix remains the same, then the next time this function is called,
